...
frappe.ui.form.on(itemDoctypName, {
  refresh: function (frm) {
    ...
    if (!frm.is_new()) {
      if (
        ...
        frm.add_custom_button(
          ...
          function () {
            ...
            frappe.call({
              method:
                'kra_etims_frappe.kra_etims.apis.apis.perform_item_registration',
              args: {
                request_data: {
                  name: frm.doc.name,
      ...
      //       frappe.call({
      //         method:
      //           'kra_etims_frappe.kra_etims.apis.apis.submit_inventory',
      //         args: {
      //           request_data: {
      //             company_name: companyName,
      //             name: frm.doc.name,
      ...
      if (
        ...
        frm.add_custom_button(
          ...
          function () {
            frappe.call({
              method:
                'kra_etims_frappe.kra_etims.apis.apis.send_imported_item_request',
              args: {
                request_data: {
                  company_name: companyName,
                  name: frm.doc.name,
