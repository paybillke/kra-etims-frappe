frappe.ui.form.on('eTims Settings', {
  refresh: function (frm) {
    ...
    if (!frm.is_new() && frm.doc.is_active) {
      frm.add_custom_button(
        ...
        function () {
          frappe.call({
            method:
              'kra_etims_frappe.kra_etims.apis.apis.perform_notice_search',
            args: {
              request_data: {
                name: frm.doc.name,
      ...
      frm.add_custom_button(
        ...
        function () {
          frappe.call({
            method:
              'kra_etims_frappe.kra_etims.apis.apis.perform_stock_movement_search',
            args: {
              request_data: {
                name: frm.doc.name,
                company_name: companyName,
    ...
    frm.add_custom_button(
      ...
      function () {
        frappe.call({
          method: 'kra_etims_frappe.kra_etims.apis.apis.ping_server',
          args: {
            request_data: {
