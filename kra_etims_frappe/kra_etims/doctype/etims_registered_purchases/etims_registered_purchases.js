...
frappe.ui.form.on(doctypeName, {
    ...
    if (!frm.is_new()) {
      frm.add_custom_button(
        ...
        function () {
          frappe.call({
            method:
              'kra_etims_frappe.kra_etims.apis.apis.create_supplier_from_fetched_registered_purchases',
            args: {
              request_data: {
                name: frm.doc.name,
      ...
      //     frappe.call({
      //       method:
      //         'kra_etims_frappe.kra_etims.apis.apis.create_items_from_fetched_registered_purchases',
      //       args: {
      //         request_data: {
      //           name: frm.doc.name,
      //           company_name: companyName,
  ...
  frappe.call({
    method: 'kra_etims_frappe.kra_etims.doctype.etims_registered_purchases.etims_registered_purchases.validate_item_mapped_and_registered',
    args: {
      items: frm.doc.items,
    },
    callback: (response) => {
    ...
    error: (error) => {
      ...
      frm.add_custom_button(
        ...
        function () {
          frappe.call({
            method:
              'kra_etims_frappe.kra_etims.apis.apis.create_purchase_invoice_from_request',
            args: {
              request_data: {
                name: frm.doc.name,
                company_name: companyName,
